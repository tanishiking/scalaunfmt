...
class Runner(err: PrintStream) {
  ...
  def run(files: Seq[File], version: String, scalaunfmtConf: Conf): Either[Throwable, String] = {
    ...
    for {
      ...
      val pathToDistance = validConfigs.par.map { candidate =>
        val distance = files.map { file =>
          val original = new String(Files.readAllBytes(file.toPath))
          val formatted = instance.withReporter(reporter).format(
            candidate.toPath,
            file.toPath,
            original
          )
  ...
  private[core] def validateConfigs(candidates: Seq[File]): Either[Throwable, Seq[File]] = {
    ...
    candidates.foreach { candidate =>
      instance.withReporter(reporter).format(
        candidate.toPath,
        dummyFilePath,
        testScalaCode
      )
...
object Runner {
  private[core] implicit class ConfOps(private val conf: Conf) extends AnyVal {
    ...
    def combination(version: String): Either[Throwable, Seq[Conf]] = {
      for {
        ls <- conf match {
          ...
          case _ => Left(new IllegalArgumentException(s"Invalid configuration, it must be obj: ${conf.show}"))
        }
        isValid = ls.forall {
          case (_, Conf.Lst(lst)) if lst.nonEmpty => true
          case _ => false
        }
        ls <- if (isValid) Right(ls) else Left(
          new IllegalArgumentException(s"Invalid configuration, its all values must be List: ${conf.show}")
        )
      } yield {
        val kvss = ls.flatMap {
    ...
    private def combinations[T](kvss: List[(String, List[T])]): List[List[(String, T)]] = kvss match {
      case Nil => List(Nil)
      case (k, vs) :: rss =>
        for {
          v <- vs
